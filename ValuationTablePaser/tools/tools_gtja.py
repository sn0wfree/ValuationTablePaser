...
from ValuationTablePaser.config.config import SELL_DIRECTION_PATTERN, ASSET_TYPE_CN_MAP, STOCK_EXCHANGE_CN_MAP, \
    ...
    FUTURE_PRODUCT_MAP, EXCHANGE_MAP, ETF_OPTIONS_NAME_PATTERN, FUTURE_OPTION_PATTERN
...
def converter_1102(row, pattern, matched):
    ...
    try:
        ...
        warnings.warn("成本价异常,使用0填充. row:%s" % row)
        price = 0
    ...
    try:
        ...
        warnings.warn("市价异常,使用成本价填充. row:%s" % row)
        mkt_price = price
    ...
    if stock_code is not None:
        ...
        if re.match(ETF_OPTIONS_NAME_PATTERN, class_name):
            ...
            if options_code is None:
                warnings.warn("合约代码转换失败. row:%s" % str(row))
            ...
            if exchange is None:
                if options_code.startswith('10'):
                    ...
                    warnings.warn("找不到交易所. row:%s" % str(row))
        ...
        if not exchange:
            if fund_code.startswith("5"):
                ...
                exchange = 'OF'
            warnings.warn(
                "根据合约代码(%s)首尾将交易所设置成%s, 可能不正确. row:%s" % (fund_code, exchange, str(row)))
...
def load_given_pattern_config(name, config):
    for c in config:
        ...
        #
        elif name in ['1002', '1021', '1031', '1108', '1109']:
            c2 = dict(zip(['1002', '1021', '1031', '1108', '1109'],
                          ['cash', 'provisions', 'margin', 'financing', 'financing']))
...
if __name__ == '__main__':
    ...
    # others
    if len(loc) == 1:
        header = data.iloc[loc.min(), :]
    else:
        header = list(level2_header_ravel(data[loc.min():loc.max()].values))
    ...
    ht = True if '.' in c4['class_id'].values.ravel()[2] else False
    ...
    for lvl1_code in lvl1code_list:
        items = list(parse_underlying(c4, lvl1_code=lvl1_code))
        h.append(items)
